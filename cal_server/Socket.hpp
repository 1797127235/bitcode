...
namespace socket_ns
{
    ...
    public:
        void CreateSocketOrDie() override
        {
            ...
            {
                LOG(ERROR_, "Failed to create TCP socket");
                throw std::runtime_error("Failed to create TCP socket");
            }
            LOG(INFO, "TCP socket created successfully, sockfd: %d", _sockfd);
        }
        void CreateBindOrDie(int16_t port) override
        {
            ...
            {
                LOG(ERROR_, "Failed to bind TCP socket to port %d", port);
                throw std::runtime_error("Failed to bind TCP socket");
            }
            LOG(INFO, "TCP socket bound to port %d successfully", port);
        }
        void CreateListenOrDie(int backlog) override
        {
            if (listen(_sockfd, backlog) == SOCKET_ERROR)
            {
                LOG(ERROR_, "Failed to listen on TCP socket");
                throw std::runtime_error("Failed to listen on TCP socket");
            }
            LOG(INFO, "TCP socket is now listening, sockfd: %d", _sockfd);
        }
        ...
        SockPtr Accepter(InetAddr *clientaddr) override
        {
            ...
            {
                LOG(ERROR_, "Client address pointer is null");
            ...
            {
                LOG(ERROR_, "Failed to accept connection on TCP socket");
                return nullptr; // 返回空指针表示接受连接失败
            }
            LOG(INFO, "Accepted connection on TCP socket, client sockfd: %d", client_sockfd);
        ...
        bool Connector(const std::string &ip, int16_t port) override
        {
            ...
            {
                LOG(ERROR_, "Invalid IP address: %s", ip.c_str());
                return false;
            }
            // 连接到服务器
            if (connect(_sockfd, (sockaddr*)&server_addr, sizeof(server_addr)) == SOCKET_ERROR)
            {
                LOG(ERROR_, "Failed to connect to server %s:%d", ip.c_str(), port);
                return false;
            }
            LOG(INFO, "Connected to server %s:%d successfully", ip.c_str(), port);
            return true;
        }
        ...
        void Close() override
        {
            ...
            {
                closesocket(_sockfd);
                LOG(INFO, "TCP socket closed, sockfd: %d", _sockfd);
